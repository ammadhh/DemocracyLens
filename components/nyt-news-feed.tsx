...
import { AlertCircle, Bookmark, Clock, ExternalLink, ThumbsDown, ThumbsUp, Newspaper } from "lucide-react"
...
export function NytNewsFeed() {
  ...
  const fetchArticles = async (reset = false) => {
    ...
    try {
      ...
      // Fetch articles from our NYT API
      const response = await fetch(`/api/nyt-news?${params.toString()}`)

      if (!response.ok) {
        throw new Error(`Failed to fetch NYT articles: ${response.status}`)
      }

      const data = await response.json()

      if (!data.success) {
        throw new Error(data.error || "Failed to fetch NYT articles")
      }
      ...
      setError(error.message || "Failed to load NYT articles")
      toast({
        title: "Error",
        description: error.message || "Failed to load NYT articles. Please try again later.",
        variant: "destructive",
      })
    } finally {
      setLoading(false)
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-2 mb-4">
        ...
        <h2 className="text-xl font-bold">New York Times</h2>
      </div>

      <Tabs defaultValue="home" value={activeSection} onValueChange={setActiveSection} className="w-full">
      ...
      {articles.length === 0 && !loading ? (
        <div className="text-center py-12">
          ...
          <p className="text-muted-foreground">Try selecting a different section</p>
        </div>
      ) : (
